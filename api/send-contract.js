...
export default async function handler(req, res) {
    ...
    try {
      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #333; padding: 20px; border: 1px solid #eaeaea; border-radius: 8px;">
        ...
        <p>Здравствуйте, <strong>${name || 'уважаемый артист'}</strong>!</p>
        <p>Ваш лицензионный договор <strong>№ ${contractNumber || 'Б/Н'}</strong> успешно сгенерирован.</p>
        <p>Пожалуйста, ознакомьтесь с ним и подпишите по ссылке ниже:</p>
        
        <div style="margin: 30px 0; text-align: center;">
          <a href="${signLink}" style="background-color: #6a1b9a; color: white; padding: 14px 28px; text-decoration: none; border-radius: 6px; font-weight: bold; display: inline-block;">Открыть и подписать договор</a>
        </div>
        
        <p style="color: #666; font-size: 14px;">Если кнопка не работает, скопируйте эту ссылку в браузер:</p>
        <p style="font-size: 13px; color: #6a1b9a; word-break: break-all; background-color: #f7effa; padding: 10px; border-radius: 4px;">${signLink}</p>
    ...
    `;
        ...
        const info = await transporter.sendMail({
            ...
            to: email,
            subject: 'Ваш договор готов к подписанию',
            html,
        });
